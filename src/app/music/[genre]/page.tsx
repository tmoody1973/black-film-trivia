...
export default function MusicPlayPage() {
  ...
  const genreDetails = useQuery(api.music.getGenreDetails, { genreId })
  const musicQuestions = useQuery(api.music.getMusicQuestions, { genreId, count: 10 })
  const startSession = useMutation(api.music.startMusicSession)
  const submitAnswer = useMutation(api.music.submitMusicAnswer)
  const completeSession = useMutation(api.music.completeMusicSession)
  const generateQuestionAction = useAction(api.generateQuestion.generateQuestion)

  // Game state
  const [sessionId, setSessionId] = useState<Id<"music_sessions"> | null>(null)
  const [currentQuestion, setCurrentQuestion] = useState<Question | null>(null)
  const [questionQueue, setQuestionQueue] = useState<Array<{ contentTitle: string; contentType: string; genre: string }>>([])
  ...
  useEffect(() => {
    if (!user || !musicQuestions || hasInitialized.current) return
    ...
    const initGame = async () => {
      try {
        setIsLoading(true)
        const newSessionId = await startSession({ genreId })
        setSessionId(newSessionId)
        setQuestionQueue(musicQuestions)

        // Generate first question
        if (musicQuestions.length > 0) {
          await generateQuestion(musicQuestions[0])
        }
      } catch (err) {
    ...
    initGame()
  }, [user, musicQuestions, genreId])

  const generateQuestion = async (content: { contentTitle: string; contentType: string; genre: string }) => {
    setIsLoading(true)
    setError(null)
    ...
    try {
      const data = await generateQuestionAction({
        ...
        contentType: content.contentType as "film" | "book" | "music",
        difficulty: "medium",
        musicGenre: content.genre,
      })
  ...
  const handleAnswer = async (isCorrect: boolean) => {
    ...
    try {
      await submitAnswer({
        sessionId,
        artistName: currentQuestion.contentTitle || currentQuestion.movieTitle || "",
        correct: isCorrect,
      })
    } catch (err) {
  ...
  }, [currentQuestionIndex, questionQueue, sessionId, completeSession, correctAnswers])
  ...
  const handlePlayAgain = async () => {
    ...
    if (user && musicQuestions) {
      try {
        ...
        const shuffled = [...musicQuestions].sort(() => Math.random() - 0.5)
  ...
  const defaultLearning: LearningContent = {
    didYouKnow: `"${getContentTitle()}" is a legendary figure in Black music history.`,
    culturalContext: `This artist represents an important contribution to ${genreDetails?.name || "Black music"}.`,
    creatorSpotlight: getCreator()
      ? `${getCreator()} has shaped the sound of a generation.`
      : `A talented artist who brought their vision to life.`,
    awards: [],
    legacy: `This artist's music continues to influence and inspire audiences today.`,
  }

  // Loading state
  if (!isUserLoaded || !genreDetails) {
    return (
      ...
      <div className="flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center space-y-8 p-4">
        ...
        >
          ...
          <p className="text-muted-foreground">
            Sign in to play Music Trivia and track your progress in {genreDetails.name}.
          </p>
          ...
          >
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/30">
                <span className="text-2xl">{genreDetails.icon}</span>
                <span className="font-display font-semibold">{genreDetails.name}</span>
              </div>
            </motion.div>
            ...
            >
              <div className={`
                ...
                bg-gradient-to-br ${genreDetails.color}
              `}>
              ...
              {score === 100 ? (
                <div className="flex items-center justify-center gap-2 text-primary">
                  ...
                  <span className="font-display font-semibold">
                    Perfect! You&apos;ve mastered {genreDetails.name}!
                  </span>
                </div>
              ) : score >= 70 ? (
                <p className="text-muted-foreground">
                  Excellent knowledge of {genreDetails.name} music!
                </p>
              ) : score >= 50 ? (
                <p className="text-muted-foreground">
                  Good start! Keep exploring {genreDetails.name}.
                </p>
              ) : (
                <p className="text-muted-foreground">
                  Every question teaches something new about {genreDetails.name} legends!
                </p>
              )}
            </motion.div>
    ...
    return (
      <div className="flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center space-y-6">
        ...
        <motion.p
          ...
          className="text-muted-foreground font-display"
        >
          Loading {genreDetails.name} trivia...
        </motion.p>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center space-y-6 py-8 px-4">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-card border border-border">
            <span className="text-lg">{genreDetails.icon}</span>
            <span className="font-display font-semibold text-sm">{genreDetails.name}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block w-24 h-2 rounded-full bg-muted overflow-hidden">
            <motion.div
              ...
              className={`h-full bg-gradient-to-r ${genreDetails.color}`}
              transition={{ type: "spring", damping: 20 }}
            />
          </div>
        </div>
      ...
      <KnowledgeReveal
        ...
        contentType="music"
        posterUrl={currentQuestion.posterUrl || currentQuestion.coverUrl || currentQuestion.albumCoverUrl}
        creator={getCreator()}
